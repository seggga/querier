...
func (s *Scanner) scanNumber() (mytoken.Token, string) {
	offs := s.offset
	tok := mytoken.ILLEGAL
...
func (s *Scanner) Scan() (pos mytoken.Pos, tok mytoken.Token, lit string) {
	...
	default:
		...
		case '<':
			...
			// 	tok = mytoken.ARROW
			// } else {
			tok = s.switch2(mytoken.LSS, mytoken.LEQ)
			// }
		case '>':
